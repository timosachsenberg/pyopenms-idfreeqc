"""ID-free QC metric computation utilities built on top of pyOpenMS.

This module originates from the ``hackathon_ID_free_metrics.ipynb`` notebook but
has been refactored so that all logic lives in reusable functions.  It can be
imported as a library or executed as a standalone script via its Click-based
...
def _rts(specs: List[oms.MSSpectrum]) -> np.ndarray:
    """Return retention times for the provided spectra."""
    return np.array([float(s.getRT()) for s in specs], dtype=float) if specs else np.array([], dtype=float)


def _mz_range(specs: List[oms.MSSpectrum]) -> Tuple[float, float]:
    """Compute overall m/z range for a collection of spectra."""
    min_mz = float("inf")
    max_mz = float("-inf")
    for spec in specs:
        try:
            min_mz = min(min_mz, float(spec.getMinMZ()))
            max_mz = max(max_mz, float(spec.getMaxMZ()))
        except Exception:
            arr = spec.getMZArray()
            if arr is None or arr.size() == 0:
                continue
            mzs = np.array([arr[i] for i in range(arr.size())], dtype=float)
            if mzs.size == 0:
                continue
            min_mz = min(min_mz, float(np.min(mzs)))
            max_mz = max(max_mz, float(np.max(mzs)))
...
def parse_mzqc_metrics(json_str: str) -> Tuple[List[str], Dict[str, Dict[str, Any]], Dict[str, List[Any]]]:
    ...
    try:
        ...
        click.echo(f"Error parsing mzQC JSON: {exc}", err=True)
        return [], {}, {}
...
def resolve_inputs(resources: Sequence[str], download_dir: Optional[Path] = None, *, verbose: bool = True) -> List[Path]:
    download_dir = Path(download_dir) if download_dir is not None else Path("downloads")
    resolved: List[Path] = []
    for resource in resources:
        if _is_url(resource):
            filename = DEMO_MZML_FILES.get(resource)
            if not filename:
                path_name = Path(urlparse(resource).path).name
                filename = path_name or f"downloaded_{len(resolved)}.mzML"
            target = download_dir / filename
            if not target.exists():
                download_dir.mkdir(parents=True, exist_ok=True)
                if verbose:
                    click.echo(f"  Downloading {resource} -> {target}")
                urlretrieve(resource, target)
            resolved.append(target)
        else:
            path = Path(resource)
            if not path.exists():
                raise FileNotFoundError(f"Input file does not exist: {resource}")
            resolved.append(path)
    return resolved


def load_experiment(path: Path) -> oms.MSExperiment:
...
def analyze_experiment(path: Path) -> Dict[str, Any]:
...
def run_idfree_qc(
    ...
    *,
    ...
    if not resources:
        resources = list(DEMO_MZML_FILES.keys())
        if verbose:
            click.echo("No inputs provided; using built-in demo dataset.")
    if verbose:
        click.echo("Processing mzML files and computing QC metrics...")
    resolved = resolve_inputs(resources, download_dir, verbose=verbose)
    ...
    for path in resolved:
        ...
        if verbose:
            click.echo(f"  ✓ Computed {len(result['metrics'])} QC metrics for {path.name}")
    if verbose:
        click.echo("\nBuilding mzQC JSON file...")
    json_str = build_mzqc(run_data)
    if output_path is not None:
        output_path = Path(output_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(json_str)
        if verbose:
            click.echo(f"\n✓ mzQC file saved to: {output_path}")
    if show_tables:
        print_metrics_tables(json_str)
    return json_str


def plot_metrics_heatmap(json_str: str, *, figsize: Optional[Tuple[float, float]] = None):
    import matplotlib.pyplot as plt
    import pandas as pd
    import seaborn as sns

    run_labels, qc_metrics_dict, _ = parse_mzqc_metrics(json_str)
    if not run_labels:
    ...
    df_heatmap = pd.DataFrame(all_metrics, index=run_labels).T
    df_heatmap = df_heatmap.dropna(how='all')
    df_original = df_heatmap.copy()

    def _scale_row(row: pd.Series) -> pd.Series:
        ...
        if std == 0:
            ...
            values[mask] = (finite_values - finite_values.mean()) / std
        values[~mask] = np.nan
        return pd.Series(values, index=row.index)

    df_scaled = df_heatmap.apply(_scale_row, axis=1, result_type='broadcast')

    def _row_to_strings(row: pd.Series) -> pd.Series:
    ...
    annot = df_original.apply(_row_to_strings, axis=1)
    figsize = figsize or (15, max(8, df_scaled.shape[0] * 0.6))
    ...
    plt.title('QC Metrics Heatmap Across Runs (Row-normalised colors)')
    plt.xlabel('Run')
    plt.ylabel('Metric')
    plt.tight_layout()
    return plt.gca()
...
@click.option(
    ...
    default=Path("multi_run_qc.mzQC.json"),
    help="Path to save the generated mzQC file.",
)
@click.option(
    "--download-dir",
    type=click.Path(file_okay=False, path_type=Path),
    default=Path("downloads"),
    help="Directory used to store downloaded mzML files.",
)
@click.option("--no-tables", is_flag=True, help="Suppress formatted metric tables in the console output.")
@click.option("--heatmap", is_flag=True, help="Display a heatmap of the computed metrics.")
def cli(resources: Tuple[str, ...], output_path: Path, download_dir: Path, no_tables: bool, heatmap: bool) -> None:
    json_str = run_idfree_qc(
        list(resources),
